...
private:
  ...
  std::pair<line*,bool> match_line(RecordType& sample);       //< Map a sample to its source line and matches with selected_line
  template<typename RecordType>
  bool based_match_line(RecordType& sample);       //< Map a sample to its source line and matches with selected_line
...
#ifdef USE_EBPF
  // Global eBPF collector (one instance for entire system)
  static struct ebpf_collector* _global_ebpf_collector;
  static bool _ebpf_initialized;
  static void distribute_ebpf_event(struct blocked_event *evt, void *ctx);
#endif
