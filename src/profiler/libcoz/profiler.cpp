...
#ifdef USE_EBPF
// Define static eBPF collector members
struct ebpf_collector *profiler::_global_ebpf_collector = nullptr;
bool profiler::_ebpf_initialized = false;
#endif
                       ...
                       bool end_to_end, char blocked_scope, bool print_log, line *based_line, char based_blocked, int based_speedup)
{
  ...
  if (blocked_scope == 'n')
    ...
    _blocked_scope = blocked_scope;

  // If based_blocked is specified, set the target based scope
  if (based_blocked == 'n')
    _based_blocked = 0;
  else
    _based_blocked = based_blocked;
...
void profiler::profiler_thread(spinlock &l)
{
...
#ifdef USE_EBPF
  // init global ebpf collector once
  if (!_ebpf_initialized)
  {
    INFO << "Initializing global eBPF collector";
    _global_ebpf_collector = ebpf_collector_create();

    REQUIRE(_global_ebpf_collector != nullptr) << "Failed to allocate eBPF collector";
    if (ebpf_collector_start(_global_ebpf_collector) != 0)
    {
      FATAL << "Failed to start eBPF collector";
    }

    // add pid filter for this process
    pid_t our_pid = getpid();
    if (ebpf_collector_add_pid(_global_ebpf_collector, our_pid) != 0)
    {
      FATAL << "Failed to add PID to eBPF filter";
    }

    _ebpf_initialized = true;
    INFO << "eBPF collector started successfully";
  }
#endif

  // wait for at least one progress point
  ...
  {
#ifdef USE_EBPF
    // poll ebpf collector for blocked samples
    if (_ebpf_initialized && _global_ebpf_collector)
    {
      ebpf_collector_poll(_global_ebpf_collector,
                          profiler::distribute_ebpf_event,
                          nullptr);
      // heartbeat so we know poll is alive
      static size_t ebpf_poll_ticks = 0;
      if ((++ebpf_poll_ticks % 200) == 0)
      {
        INFO << "eBPF poll heartbeat (" << ebpf_poll_ticks << ")";
      }
    }
#endif
    ...
    if (_blocked_scope != 0)
    {
      REQUIRE(_blocked_scope == 'a' || _blocked_scope == 'b' ||
              ...
              _blocked_scope == 'i' || _blocked_scope == 'o')
          << "No such blocked_scope!";
    ...
    else if (_based_blocked)
    {
      REQUIRE(_based_blocked == 'a' || _based_blocked == 'b' ||
              ...
              _based_blocked == 'i' || _based_blocked == 'o')
          << "No such blocked_scope!";
    ...
    for (const auto &s : saved_latency_points)
    {
      ...
      if (begin_delta < min_delta)
        min_delta = begin_delta;
...
// Create this thread's perf_event sampler and start sampling (on-CPU)
...
bool profiler::based_match_line(RecordType &sample)
{
  line *l;
...
template <typename RecordType>
std::pair<line *, bool> profiler::match_line(RecordType &sample)
{
...
void profiler::process_blocked_samples(thread_state *state)
{
  ...
  auto handle_perf_record = [&](perf_event::record &r) {
    if (!r.is_sample())
      return;

    // perf uses task-clock
    REQUIRE(r.get_time() >= state->last_perf_time) << "Already processed on-CPU sample!";
    state->last_perf_time = r.get_time();
...
#ifdef USE_EBPF
  // ebpf sample handler (ktime domain)
  auto handle_blocked_record = [&](ebpf_record &r) {
    if (!r.is_sample())
      return;

    // ebpf timestamps are ktime; keep separate monotonic check
    if (r.get_time() < state->last_ebpf_time)
    {
      // log once if clock goes backwards; continue
      static std::atomic<bool> warned{false};
      if (!warned.exchange(true))
      {
        INFO << "eBPF sample time went backwards (last=" << state->last_ebpf_time
             << " curr=" << r.get_time() << ")";
      }
    }
    state->last_ebpf_time = r.get_time();
  ...
  for (auto r : state->sampler)
  {
    handle_perf_record(r);
  }

#ifdef USE_EBPF
  for (auto &evt : state->blocked_events)
  {
    ebpf_record r(evt);
    handle_blocked_record(r);
  }
...
void profiler::process_samples(thread_state *state)
{
  ...
  // On-CPU samples from perf_event
  for (auto r : state->sampler)
  {
    handle_perf_record(r);
  }
...
#ifdef USE_EBPF
  for (const auto &evt : state->blocked_events)
  {
    ebpf_record r(evt);
    handle_blocked_record(r);
  }
...
#ifdef USE_EBPF
/**
 * Distribute eBPF events to thread-local buffers
 * Called by eBPF collector for each blocked sample event
 */
void profiler::distribute_ebpf_event(struct blocked_event *evt, void *ctx)
{
  profiler &p = profiler::get_instance();

  pid_t tid = evt->tid;

  // static_map::find returns a pointer to thread_state or nullptr
  thread_state *state = p._thread_states.find(tid);
  if (state == nullptr)
  {
    // Either thread not yet registered or already exited
    return;
  }

  static std::atomic<uint64_t> ebpf_events_seen{0};
  uint64_t cnt = ++ebpf_events_seen;
  if ((cnt % 50) == 0)
  {
    INFO << "eBPF events delivered: " << cnt << " (latest tid=" << tid
         << " type=" << static_cast<int>(evt->blocked_type)
         << " dur_ns=" << evt->duration_ns << ")";
  }

  state->blocked_events.push_back(*evt);
}
#endif

// Explicit template instantiations for both record types
#ifdef USE_EBPF
#include "ebpf_adapter.h"
template bool profiler::based_match_line<ebpf_record>(ebpf_record &);
template std::pair<line *, bool> profiler::match_line<ebpf_record>(ebpf_record &);
#else
#include "perf.h"
template bool profiler::based_match_line<perf_event::record>(perf_event::record &);
template std::pair<line *, bool> profiler::match_line<perf_event::record>(perf_event::record &);
#endif
